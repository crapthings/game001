import { applyOpeningProgress } from './opening/openingProgress.js'
...
export function applyProgress(world, progress, event) {
  if (event.type === 'opening') return applyOpeningProgress(world, progress, event)
  ...
  if (event.type === 'checkpoint') {
    if (event.combat !== undefined && !validCombat(event.combat)) throw new Error('武器状态无效。')
    ...
    return { ...progress, ...(event.combat !== undefined ? { combat: structuredClone(event.combat) } : {}), playerPosition: [...event.position], exploredFog, ...(event.stamina !== undefined ? { stamina: { ...event.stamina } } : {}), ...(event.worldTime !== undefined ? { worldTime: event.worldTime } : {}), ...(event.flashlight !== undefined ? { flashlight: { ...event.flashlight } } : {}) }
  }
