...
export function createZombieModel(scene, definition) {
  ...
  const strideAngle = crawling ? .32 : d.id === 'runner' ? .58 : d.build > 1.3 ? .34 : .42
  let time = 0, walkPhase = 0
  function pose(moving = false, running = false) {
    const stride = moving ? strideAngle : 0
    ...
    for (let i=0;i<2;i++) {
      const swing = Math.sin(walkPhase+i*Math.PI)
