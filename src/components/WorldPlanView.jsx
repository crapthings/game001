import { sampleEnvironment } from '../game/world/biomes/environmentField.js'
import OpeningPlanOverlay, { PlanningGridOverlay } from './OpeningPlanOverlay.jsx'
import { createTerrain } from '../game/world/chunks/terrain.js'
...
export default function WorldPlanView({plan,selected,onSelect,focus,layers,baseMap='ecology'}) {
  const ecology = useMemo(() => {
    ...
    const cells = []
    const finalTerrain = ['finalHeight', 'groves'].includes(baseMap) ? createTerrain(plan.seed, plan.settlements, plan) : null
    ...
    for (let z = -1024; z < 1024; z += 32) for (let x = -1024; x < 1024; x += 32) {
      ...
      cells.push({ x, z, terrain: baseMap === 'groves' && finalTerrain ? { groves: sampleEnvironment(finalTerrain, x + 16, z + 16).treeDensity } : finalTerrain ? { height: finalTerrain.surfaceHeight(x + 16, z + 16) } : topography?.sample(x + 16, z + 16), color: `rgb(${sample.color.map(value => Math.round(value * 255)).join(',')})` })
    }
    return cells
  }, [plan, baseMap])
  ...
  return <svg role="img" aria-label={`种子 ${plan.seed} 的世界区域规划，北方为 +Z`} viewBox={`${bounds.minX-pad} ${-bounds.maxZ-pad} ${spanX+pad*2} ${spanZ+pad*2}`} className="h-full w-full select-none bg-[#0c1314]">
    ...
    {baseMap !== 'regions' && ecology.map(cell => <rect key={`${cell.x}:${cell.z}`} x={cell.x} y={-cell.z-32} width={32.2} height={32.2} fill={baseMap === 'ecology' || !cell.terrain ? cell.color : terrainColor(baseMap === 'finalHeight' ? 'height' : baseMap, cell.terrain)} pointerEvents="none"/>)}
    ...
    <g pointerEvents="none">
      {layers.scenes && <PlanningGridOverlay level={16} line={line}/>}
      {layers.scatter && <PlanningGridOverlay level={32} line={line}/>}
      ...
      {layers.opening && <OpeningPlanOverlay opening={plan.opening} grid={plan.planningGrid} line={line}/>}
